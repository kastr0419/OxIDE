...
fn avr_boards_use_avrdude() {
    for p in BOARD_PRESETS {
        if p.target_triple.starts_with("avr") {
            assert!(
                matches!(p.flash_tool, FlashToolKind::Avrdude),
                "AVR board '{}' should use avrdude",
                p.display_name
            );
        }
    }
    ...
    for p in BOARD_PRESETS {
        if p.target_triple.starts_with("xtensa-esp")
            || (p.target_triple.starts_with("riscv")
                && matches!(
                    p.kind,
                    BoardKind::Esp32C3 | BoardKind::Esp32C6 | BoardKind::Esp32H2
                ))
        {
            assert!(
                matches!(p.flash_tool, FlashToolKind::Esptool),
                "ESP board '{}' should use esptool",
                p.display_name
            );
        }
    }
    ...
    for p in BOARD_PRESETS {
        if p.target_triple.starts_with("thumbv") {
            assert!(
                ...
                "Cortex-M board '{}' should use a valid flash tool",
                p.display_name
            );
        }
    }
...
fn board_count_at_least_four() {
    assert!(
        BOARD_PRESETS.len() >= 4,
        "Should have at least 4 boards, got {}",
        BOARD_PRESETS.len()
    );
}
