...
Files:
- alloide_setup.iss  : Inno Setup 6 script for the installer
...
2. Ensure cargo is available to build the release binary, or place D:\ALLoIDE\target\release\alloide.exe manually.
3. Open an elevated PowerShell and run:
   .\build_installer.ps1

Notes
- Implementation identifiers: `alloide.exe` is the binary, `ALLoIDE_Setup_0.1.0.exe` is the installer output, `ALLoIDE` is the installer/shortcut display name, and `{pf}\ALLoIDE` is the install directory.
...
- The installer adds {pf}\ALLoIDE to the system PATH and creates shortcuts.
