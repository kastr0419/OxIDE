...
fn pin_color(fns: &[PinFunction]) -> Color32 {
    fns.first().map(|&f| func_color(f)).unwrap_or(Color32::from_rgb(170, 170, 170))
}
...
fn ui_legend_and_filter(ui: &mut egui::Ui, filter: &mut u8) {
    ui.horizontal_wrapped(|ui| {
        ...
        if ui.add(sel).clicked() { *filter = 0; }
        for (id, label, func) in FILTER_ENTRIES {
            let color = func_color(*func);
            let text = RichText::new(*label)
                .color(if *filter == *id { Color32::WHITE } else { color })
                .strong();
            let btn = egui::Button::new(text)
                .fill(if *filter == *id { color.linear_multiply(0.8) } else { Color32::TRANSPARENT })
...
fn ui_diagram(
    ...
    painter.rect_stroke(rect, 8.0, Stroke::new(1.5, Color32::from_rgb(70, 90, 100)), egui::StrokeKind::Inside);
    ...
    for p in pins.iter() {
        ...
        let alpha_color = if visible { color } else { color.linear_multiply(0.18) };

        // Circle
        painter.circle_filled(center, pin_r, alpha_color);
        let is_selected = hovered.map(|n| n == p.number).unwrap_or(false);
        let stroke_color = if is_selected { Color32::WHITE } else { Color32::BLACK };
...
fn ui_table(ui: &mut egui::Ui, pins: &[PinInfo], filter: u8, hovered: &mut Option<u8>) {
    ...
    TableBuilder::new(ui)
        ...
        .header(20.0, |mut row| {
            row.col(|ui| { ui.strong("#"); });
            row.col(|ui| { ui.strong("名前"); });
            row.col(|ui| { ui.strong("機能"); });
        })
        .body(|mut body| {
            for p in pins.iter().filter(|p| pin_visible(p.functions, filter)) {
                ...
                body.row(20.0, |mut row| {
                    row.col(|ui| {
                        let txt = RichText::new(format!("{}", p.number))
                            .color(if is_sel { Color32::YELLOW } else { Color32::GRAY });
                        ui.label(txt);
                    });
                    row.col(|ui| {
                    ...
                    row.col(|ui| {
                        ui.horizontal(|ui| {
                            for &f in p.functions {
                                let c = func_color(f);
                                let badge = RichText::new(func_label(f))
                                    .color(c)
                                    .small()
                                    .strong();
                                ui.label(badge);
...
fn ui_pin_detail(ui: &mut egui::Ui, pins: &[PinInfo], hovered: Option<u8>) {
    let Some(num) = hovered else { return; };
    let Some(p)   = pins.iter().find(|pp| pp.number == num) else { return; };

    ui.separator();
    egui::Frame::group(ui.style()).show(ui, |ui| {
        ui.horizontal(|ui| {
            ui.strong(format!("📌  {}", p.name));
            ui.label(RichText::new(format!("(Pin {})", p.number)).color(Color32::GRAY).small());
        });
        ui.add_space(4.0);
        ui.horizontal_wrapped(|ui| {
            for &f in p.functions {
...
pub fn ui_pinout_panel(app: &mut IdeApp, ui: &mut egui::Ui) {
    let board = crate::core::board::BOARD_PRESETS.get(app.selected_board).cloned();
    ...
    ui.vertical(|ui| {
        ...
        // ── Filter ──────────────────────────────────────────────────────────
        let visible_count = pinout.pins.iter()
            .filter(|p| pin_visible(p.functions, app.pinout_filter))
            .count();
        ui.horizontal(|ui| {
            ui.label(RichText::new(format!("{} ピン表示中", visible_count)).small().color(Color32::GRAY));
        });
        ...
        if app.pinout_view_table {
            ui_table(ui, pinout.pins, app.pinout_filter, &mut app.pinout_hovered_pin);
        } else {
            ui_diagram(ui, pinout.pins, app.pinout_filter, &mut app.pinout_hovered_pin);
        }
