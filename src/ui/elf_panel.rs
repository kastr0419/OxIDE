...
pub fn ui_elf_panel(app: &mut IdeApp, ui: &mut Ui) {
        ...
        if ui.button("Analyze ELF").clicked() {
            ...
            std::thread::spawn(move || {
                if let Some(board) = board {
                    if let Some(elf_path) = crate::core::build_analyzer::find_elf(&project_dir, &board) {
                        match crate::core::elf_analyzer::analyze_elf(&elf_path) {
                            Ok(info) => { let _ = tx.send(crate::app::AppMessage::ElfAnalysis(info)); }
                            Err(e) => { let _ = tx.send(crate::app::AppMessage::Error(e.to_string())); }
                        }
                    } else {
                        let _ = tx.send(crate::app::AppMessage::Error("ELF not found".to_string()));
                    }
                } else {
                    let _ = tx.send(crate::app::AppMessage::Error("No board selected".to_string()));
                }
            });
        }
    });
    ...
    if let Some(ref info) = app.elf_info {
        ...
        egui::ScrollArea::vertical().max_height(200.0).show(ui, |ui| {
            egui::Grid::new("elf_sections_grid").striped(true).min_col_width(60.0).show(ui, |ui| {
                ui.label("Name"); ui.label("VMA"); ui.label("Size"); ui.label("Type"); ui.end_row();
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            egui::Grid::new("elf_symbols_grid").striped(true).min_col_width(80.0).show(ui, |ui| {
                ui.label("Name"); ui.label("Address"); ui.label("Size"); ui.label("Type"); ui.end_row();
                for sym in &info.symbols {
                    if !filter.is_empty() && !sym.name.contains(&filter) { continue; }
                    ui.label(&sym.name);
        ...
        ui.colored_label(egui::Color32::YELLOW, "ビルド後に「Analyze ELF」をクリックしてください");
    }
}
