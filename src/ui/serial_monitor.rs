...
pub fn ui_serial_monitor(app: &mut crate::app::IdeApp, ui: &mut egui::Ui, tx: &crossbeam_channel::Sender<crate::app::AppMessage>) {
    ui.horizontal(|ui| {
        if app.is_serial_connected {
            ...
            if ui.button("Connect").clicked() {
                ...
                let baud = crate::core::board::BOARD_PRESETS.get(app.selected_board).map(|p| p.default_baud).unwrap_or(crate::core::config::DEFAULT_BAUD_RATE);
                let settings = SerialSettings { port_name: port.clone(), baud_rate: baud };
        ...
        ui.separator();
        ui.label("Baud:");
        egui::ComboBox::from_id_salt("baud_cb").selected_text("Baud").show_ui(ui, |ui| {
    ...
    ui.horizontal(|ui| {
        if ui.selectable_label(!app.show_plotter_tab, "📟 Monitor").clicked() {
            app.show_plotter_tab = false;
        }
        if ui.selectable_label(app.show_plotter_tab, "📈 Plotter").clicked() {
    ...
    ui.horizontal(|ui| {
        ...
        if (resp.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter))) || ui.button("Send").clicked() {
            if let Some(tx_cmd) = &app.serial_tx {
                let _ = tx_cmd.send(crate::core::serial::SerialCommand::Send(app.serial_input.clone()));
            }
