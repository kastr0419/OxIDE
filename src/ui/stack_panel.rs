...
pub fn ui_stack_panel(app: &mut IdeApp, ui: &mut Ui) {
    ui.vertical(|ui| {
                    ...
                    if let Some(board) = board {
                        if let Some(elf_path) = crate::core::build_analyzer::find_elf(&project_dir, &board) {
                            match crate::core::stack_analyzer::analyze_stack(&elf_path) {
                                Ok(report) => { let _ = tx.send(crate::app::AppMessage::StackAnalysis(report)); }
        ...
        if let Some(report) = &app.stack_report {
            ui.label(format!("Estimated total stack: {} bytes", report.total_estimate));
            if !report.frames.is_empty() {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for f in &report.frames {
                        if !app.stack_filter.is_empty() && !f.function.contains(&app.stack_filter) { continue; }
                        ui.horizontal(|ui| {
                            ui.label(f.function.to_string());
                            if let Some(s) = f.stack_usage { ui.label(format!("{} bytes", s)); }
                        });
                    }
                });
            } else {
