...
pub fn parse_svd(xml: &str) -> Result<SvdDevice> {
    ...
    if let Some(peripherals_node) = root.children().find(|n| n.has_tag_name("peripherals")) {
        for periph_node in peripherals_node.children().filter(|n| n.has_tag_name("peripheral")) {
            ...
            let pdesc = find_text(&periph_node, "description").unwrap_or("").to_string();
            ...
            if let Some(regs_node) = periph_node.children().find(|n| n.has_tag_name("registers")) {
                for reg_node in regs_node.children().filter(|n| n.has_tag_name("register")) {
                    ...
                    let rdesc = find_text(&reg_node, "description").unwrap_or("").to_string();
                    let offset = find_text(&reg_node, "addressOffset")
                        .and_then(parse_hex_or_dec)
                        .unwrap_or(0);
                    let access = find_text(&reg_node, "access").unwrap_or("read-write").to_string();

                    let mut fields = Vec::new();
                    if let Some(fields_node) = reg_node.children().find(|n| n.has_tag_name("fields")) {
                        for field_node in fields_node.children().filter(|n| n.has_tag_name("field")) {
                            ...
                            let fdesc = find_text(&field_node, "description").unwrap_or("").to_string();
                            ...
                            let bit_width = find_text(&field_node, "bitWidth")
                                .and_then(|s| s.parse::<u8>().ok())
                                .unwrap_or(1);
                            fields.push(SvdField { name: fname, description: fdesc, bit_offset, bit_width });
                        }
                    }
                    registers.push(SvdRegister { name: rname, description: rdesc, address_offset: offset, access, fields });
                }
            }
            peripherals.push(SvdPeripheral { name: pname, description: pdesc, base_address: base_addr, registers });
        }
    }
